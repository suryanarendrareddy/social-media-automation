          ...
          {PLATFORMS.map((p) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 rounded-xl border text-left transition-all p-3.5 ${isConnected? "border-emerald-200 bg-emerald-50 cursor-default" : "border-slate-200 bg-slate-50 hover:border-slate-300 hover:bg-slate-100 cursor-pointer"} ${isConnecting && "opacity-60"}`}
              >
                <div className="p-2">
                  <p.icon
                    className={`size-5 ${isConnected ? "text-emerald-600" : "text-slate-500"}`}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <div
                    className={`text-sm ${isConnected ? "text-emerald-700" : "text-slate-800"}`}
                  >
                    {p.name}
                  </div>
                ...
                {isConnecting && (
                  <div className="size-4 border-red-600 border-t-transparent rounded-full animate-spin shrink-0" />
                )}
